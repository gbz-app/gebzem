...
class KanalServisi {
  ...
  Future<String> olustur({
    ...
    final r = await _api.post('/channels', data: {
      'ad': ad,
  ...
  Future<void> duzenle(String id,
      {required String ad,
      String aciklama = '',
      String avatarMediaId = ''}) async {
    await _api.patch('/channels/$id', data: {
      'ad': ad,
      'aciklama': aciklama,
      'avatar_media_id': avatarMediaId,
    });
  ...
  Future<List<Kanal>> kesfet({String q = ''}) async {
    final r = await _api.get('/channels/kesfet',
  ...
  Future<void> abonelikBirak(String id) => _api.delete('/channels/$id/subscribe');
  ...
  Future<List<KanalGonderi>> gonderiler(String id, {String? before}) async {
    final r = await _api.get('/channels/$id/posts',
  ...
  Future<int> gonderiPaylas(String id,
      {String metin = '', List<String> mediaIds = const []}) async {
    final r = await _api.post('/channels/$id/posts', data: {
      'metin': metin,
      'media_ids': mediaIds,
    });
