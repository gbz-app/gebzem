...
- YOK — **SENKRON SAYAC SURUMU YAYINLANDI (20 Tem 14:55):** 1:1 sure sayaci artik YEREL SES yerine "baglanti kuruldu + peer odada + sunucu-aktif (elapsed_ms)" anina bagli (WhatsApp modeli) -> kilitli/CallKit yolunda iki taraf SENKRON sayar (asimetri <0.5sn); sure DEGER mantigi (Stopwatch/elapsed_ms) DOKUNULMADI, grup referanssiz -> eski enerji/8sn yolu AYNEN. ONCEKI surumler canli (yeni arayuz, anlik liste, konuk-split, hiz). **Kullanici test edecek.** ⚠️ SURE SAYAC TETIGI: 1:1 _mediaBaslat = _odaBagli && _peerJoined && _sureReferansVar (3 gelis sirasi: _odayaBaglan sonu + ParticipantConnected + _sureReferansiAl); ses sagligi AYRI (AppDelegate toggle + kurtarma aglari). Ring fazinda setSpeakerOn/mic/_sesiAc YASAK.
