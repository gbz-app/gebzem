...
- YOK — **YENI ARAYUZ + ANLIK LISTE SURUMU YAYINLANDI (20 Tem 03:35):** koyu tema (siyah alt menu, dairesiz/yazisiz/buyuk nav ikonlari aktif-beyaz/pasif-gri, Gebzem alti arama input, mor-gradient + FAB, ust-sag +); konuk atinca siyah/patlama SINYAL-gate ile bitti; yayin ust bar kompakt (overflow); kilitli-sayac gercek playout-enerjisiyle basliyor (fast-path taze>0 kaldirildi, tum iOS delta-sartli); ANLIK kesfet (hub Broadcast + stream/room.list.changed WS -> live_tab/rooms_tab invalidate, sayfa yenilemeden). Backend deploy edildi. **Kullanici test edecek.** ⚠️ Ring fazinda setSpeakerOn/mic/_sesiAc YASAK; hub broadcast SEYREK olaylar icin (50K olcekte presence-abonelik gerekir).
