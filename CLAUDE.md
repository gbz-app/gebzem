...
- YOK — **KONUK-PANEL + YENI MENU SURUMU YAYINLANDI (20 Tem 19:05):** canli yayinda konuk SERT-KAPATINCA panel artik kalkiyor (broadcast+viewer ParticipantDisconnected identity-kapili -> ~10-20sn LiveKit kopma tespiti; nazik/kick guest.left ile aninda); alt menu sade (tap dairesi kaldirildi NoSplash+overlay transparent, sol/sag ust-kose radius 20); arama alti "Sik gorustuklerin" profil seridi. ONCEKI surumler canli (senkron sayac, hiz, anlik liste). **iOS SISTEM PiP: flutter_webrtc iOS PiP SAGLAMAZ (pod'da sifir sembol) -> SDK sinirlamasi, agir belirsiz native R&D; ALINMADI. Android PiP tam calisiyor.** **Kullanici test edecek.** ⚠️ KONUK-SPLIT: SINYAL-bazli (_konukId/_aktifKonuk); guest.left + ParticipantDisconnected temizler; konukVar track-OR YAPMA (siyah bug diriltir).
