...
func TestEtkinlikSutunlariScanIleUyumlu(t *testing.T) {
	...
	if sutun != scan {
		t.Fatalf("SUTUN/SCAN UYUSMAZLIGI: `const sutunlar` %d oge, "+
			...
			"atlanir ve etkinlik listesi BOMBOS doner.", sutun, scan)
	}
	...
	elleSelect := strings.Count(kaynak, "SELECT e.id")
	if elleSelect > 0 {
		t.Fatalf("ELLE YAZILMIS %d adet `SELECT e.id` var — `sutunlar` "+
