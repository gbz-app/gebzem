...
type Handler struct {
	db  *pgxpool.Pool
	cfg *config.Config
	sms *sms.Sender // nil ise test modu (kod yanitta doner)
}

func NewHandler(db *pgxpool.Pool, cfg *config.Config, smsSender *sms.Sender) *Handler {
...
func (h *Handler) Register(w http.ResponseWriter, r *http.Request) {
	...
	// SMS saglayicisi tanimliysa gercek SMS gonder
	if h.sms.Enabled() {
	...
	if !h.sms.Enabled() && h.cfg.DevMode {
		resp["dev_otp"] = otp // test modu: SMS yok, kod yanitta doner
	}
